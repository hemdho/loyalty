	...
	public RestResponseEntityExceptionHandler() {
	 ...
	 @ExceptionHandler({org.springframework.security.access.AccessDeniedException.class})
	    protected ResponseEntity<Object> handleAccessDeniedException(final RuntimeException ex, final WebRequest request){
	    	  ...
	    	  final GenericResponse bodyOfResponse = new GenericResponse(messages.getMessage("message.unauth", null, request.getLocale()), "UnAtuhorised Access");
	    ...
	    protected ResponseEntity<Object> handleIllegalArgumentException(final RuntimeException ex,final WebRequest request){
	    	logger.error("400 Status Code", ex);
	    	 final GenericResponse bodyOfResponse = new GenericResponse(messages.getMessage("message.illegalArgument", null, request.getLocale()), "");
...
*/
	 ...
	 /*   @ExceptionHandler({ UserAlreadyExistException.class })
	    ...
	    public ResponseEntity<Object> handleRecordAlreadyExist(final RuntimeException ex, final WebRequest request) {
	        logger.error("409 Status Code", ex);
	        final GenericResponse bodyOfResponse = new GenericResponse(ex.getMessage(), "RecordAlreadyExist ");
	        return handleExceptionInternal(ex, bodyOfResponse, new HttpHeaders(), HttpStatus.CONFLICT, request);
	    }
	  ...
	  /*  @ExceptionHandler({ MailAuthenticationException.class })
	   ...
	   /* @ExceptionHandler({ ReCaptchaUnavailableException.class })
	    ...
	    public ResponseEntity<Object> handleBadCredentials(final RuntimeException ex,final WebRequest request){
	        ...
	        final GenericResponse bodyOfResponse = new GenericResponse(messages.getMessage("message.badCredentials", null, request.getLocale()), "InternalError");
